...
const filtred = numbers.filter(isEven);
...
function multiply(...numbers) {
	return numbers.reduce((prod, value) => {
 return prod * value;
}, 1)
}
...
console.log(div3(3))
const mapped3 = numbers.map(div3);
console.log(`"mapped3: ${mapped3}`);


const divisibleB = (divisor) => (x) => x % divisor === 0;

console.log(divisibleB(3)(2));
...
const incr1002 = increment(100)(2);

const increment2 = (init) => (step) => () => init += step;
...
function colorCycle(argument) {
    const c = argument || COLOR_CYCLE_DEFAULT;
    let index = 0;
    return function() {
    	index++;
...
const range2 = n => Array.from({length: n}, (v, i) => i);

console.log('range(10)', range(10));
console.log('range2(10)', range2(10));
console.log('range(100) des divisible par 13', range(100).filter(divisibleBy(13)));
...
function randomArray(N, min_val=0, max_val=100) {
	return Array.from({length: N}, (v, i) => randomInRange(min_val, max_val));

}

console.log('randomInrange without arguments : ',randomInRange());
console.log('randomInrange : ',randomInRange(250,550));
...
*/
const reducer = (array, char) => {
        if (typeof array[char] === "undefined")
            array[char] = 0;
        array[char] ++;
        return array;
    };
 function countOccurrences(string) {
	return Array.from(string).reduce(reducer,{});
}
...
function normalizeCounts(array) {
    const somme = Object.keys(array).reduce((prev, char) =>  prev + array[char], 0);
    Object.keys(array).forEach(key => array[key] = array[key] / somme);
    return array;	
}


function countOccurencesNormalized(source) {
	return normalizeCounts(countOccurrences(source));
} 
